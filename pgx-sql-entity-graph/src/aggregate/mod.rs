    ...
    pub fn new(mut item_impl: ItemImpl) -> Result<CodeEnrichment<Self>, syn::Error> {
        ...
        let fn_state_name = if let Some(found) = fn_state {
            ...
            pg_externs.push(parse_quote! {
                ...
                fn #fn_name(this: #type_state_without_self, #(#args_with_names),*, fcinfo: ::pgx::pg_sys::FunctionCallInfo) -> #type_state_without_self {
                    <#target_path as ::pgx::aggregate::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ::pgx::aggregate::Aggregate>::state(this, (#(#arg_names),*), fcinfo)
                    )
                }
            });
            fn_name
        ...
        let fn_combine_name = if let Some(found) = fn_combine {
            ...
            pg_externs.push(parse_quote! {
                ...
                fn #fn_name(this: #type_state_without_self, v: #type_state_without_self, fcinfo: ::pgx::pg_sys::FunctionCallInfo) -> #type_state_without_self {
                    <#target_path as ::pgx::aggregate::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ::pgx::aggregate::Aggregate>::combine(this, v, fcinfo)
                    )
                }
            });
            Some(fn_name)
        ...
        let fn_finalize_name = if let Some(found) = fn_finalize {
            ...
            if direct_args_with_names.len() > 0 {
                pg_externs.push(parse_quote! {
                    #[allow(non_snake_case, clippy::too_many_arguments)]
                    #pg_extern_attr
                    fn #fn_name(this: #type_state_without_self, #(#direct_args_with_names),*, fcinfo: ::pgx::pg_sys::FunctionCallInfo) -> #type_finalize {
                        <#target_path as ::pgx::aggregate::Aggregate>::in_memory_context(
                            fcinfo,
                            move |_context| <#target_path as ::pgx::aggregate::Aggregate>::finalize(this, (#(#direct_arg_names),*), fcinfo)
                        )
                    }
                });
            } else {
                pg_externs.push(parse_quote! {
                    #[allow(non_snake_case, clippy::too_many_arguments)]
                    #pg_extern_attr
                    fn #fn_name(this: #type_state_without_self, fcinfo: ::pgx::pg_sys::FunctionCallInfo) -> #type_finalize {
                        <#target_path as ::pgx::aggregate::Aggregate>::in_memory_context(
                            fcinfo,
                            move |_context| <#target_path as ::pgx::aggregate::Aggregate>::finalize(this, (), fcinfo)
                        )
                    }
                });
            };
            ...
            item_impl.items.push(parse_quote! {
                fn finalize(current: Self::State, direct_args: Self::OrderedSetArgs, _fcinfo: ::pgx::pg_sys::FunctionCallInfo) -> #type_finalize {
        ...
        let fn_serial_name = if let Some(found) = fn_serial {
            ...
            pg_externs.push(parse_quote! {
                ...
                fn #fn_name(this: #type_state_without_self, fcinfo: ::pgx::pg_sys::FunctionCallInfo) -> Vec<u8> {
                    <#target_path as ::pgx::aggregate::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ::pgx::aggregate::Aggregate>::serial(this, fcinfo)
                    )
                }
            });
            Some(fn_name)
        ...
        let fn_deserial_name = if let Some(found) = fn_deserial {
            ...
            pg_externs.push(parse_quote! {
                #[allow(non_snake_case, clippy::too_many_arguments)]
                #pg_extern_attr
                fn #fn_name(this: #type_state_without_self, buf: Vec<u8>, internal: ::pgx::pgbox::PgBox<#type_state_without_self>, fcinfo: ::pgx::pg_sys::FunctionCallInfo) -> ::pgx::pgbox::PgBox<#type_state_without_self> {
                    <#target_path as ::pgx::aggregate::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ::pgx::aggregate::Aggregate>::deserial(this, buf, internal, fcinfo)
                    )
                }
            });
            Some(fn_name)
            ...
            item_impl.items.push(parse_quote! {
                fn deserial(current: #type_state_without_self, _buf: Vec<u8>, _internal: ::pgx::pgbox::PgBox<#type_state_without_self>, _fcinfo: ::pgx::pg_sys::FunctionCallInfo) -> ::pgx::pgbox::PgBox<#type_state_without_self> {
        ...
        let fn_moving_state_name = if let Some(found) = fn_moving_state {
            ...
            pg_externs.push(parse_quote! {
                ...
                fn #fn_name(
                    ...
                    <#target_path as ::pgx::aggregate::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ::pgx::aggregate::Aggregate>::moving_state(mstate, (#(#arg_names),*), fcinfo)
                    )
                }
            });
            Some(fn_name)
            ...
            item_impl.items.push(parse_quote! {
                fn moving_state(
                    _mstate: <#target_path as ::pgx::aggregate::Aggregate>::MovingState,
                    _v: Self::Args,
                    _fcinfo: ::pgx::pg_sys::FunctionCallInfo,
                ) -> <#target_path as ::pgx::aggregate::Aggregate>::MovingState {
        ...
        let fn_moving_state_inverse_name = if let Some(found) = fn_moving_state_inverse {
            ...
            pg_externs.push(parse_quote! {
                ...
                fn #fn_name(
                    ...
                    <#target_path as ::pgx::aggregate::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ::pgx::aggregate::Aggregate>::moving_state_inverse(mstate, (#(#arg_names),*), fcinfo)
                    )
                }
            });
            Some(fn_name)
        ...
        let fn_moving_finalize_name = if let Some(found) = fn_moving_finalize {
            ...
            pg_externs.push(parse_quote! {
                #[allow(non_snake_case, clippy::too_many_arguments)]
                #pg_extern_attr
                fn #fn_name(mstate: #type_moving_state, #(#direct_args_with_names),* #maybe_comma fcinfo: ::pgx::pg_sys::FunctionCallInfo) -> #type_finalize {
                    <#target_path as ::pgx::aggregate::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ::pgx::aggregate::Aggregate>::moving_finalize(mstate, (#(#direct_arg_names),*), fcinfo)
                    )
                }
            });
            Some(fn_name)
